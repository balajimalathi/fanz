...
interface Conversation {
  ...
  requestStatus?: "pending_request" | "accepted" | "rejected";
...
export function InboxPageClient({ creatorId, currentUserId }: InboxPageClientProps) {
  ...
  const handleAcceptRequest = async (conversationId: string) => {
    try {
      ...
      setConversations((prev) =>
        prev.map((c) =>
          c.id === conversationId
            ? { ...c, requestStatus: updatedConv.requestStatus, acceptedAt: updatedConv.acceptedAt }
            : c
        )
      );
      ...
      const updatedMap = new Map(conversationMap);
      const conv = updatedMap.get(updatedConv.fanId);
      if (conv) {
        updatedMap.set(updatedConv.fanId, {
          ...conv,
          requestStatus: updatedConv.requestStatus,
          acceptedAt: updatedConv.acceptedAt,
        });
        setConversationMap(updatedMap);
      }
    } catch (error) {
  ...
  const handleRejectRequest = async (conversationId: string) => {
    try {
      ...
      setConversations((prev) =>
        prev.map((c) =>
          c.id === conversationId
            ? { ...c, requestStatus: updatedConv.requestStatus, rejectedAt: updatedConv.rejectedAt }
            : c
        )
      );
      ...
      if (conv) {
        updatedMap.set(updatedConv.fanId, {
          ...conv,
          requestStatus: updatedConv.requestStatus,
          rejectedAt: updatedConv.rejectedAt,
        });
        setConversationMap(updatedMap);
      }
    } catch (error) {
  ...
  const pendingRequests = conversations.filter((c) => c.requestStatus === "pending_request");
  const acceptedConversations = conversations.filter((c) => c.requestStatus === "accepted" || !c.requestStatus);
          ...
          {followers.length === 0 ? (
            ...
            <div className="divide-y">
              ...
              {pendingRequests.length > 0 && (
                <>
                  <div className="p-2 bg-yellow-50 dark:bg-yellow-950 border-b">
                    <p className="text-xs font-semibold text-yellow-800 dark:text-yellow-200">
                      Pending Requests ({pendingRequests.length})
                    </p>
                  </div>
                  {pendingRequests.map((conv) => {
                    const follower = followers.find((f) => f.followerId === conv.otherUserId);
                  ...
                  {acceptedConversations.length > 0 && (
                    <div className="p-2 bg-muted border-b">
                      <p className="text-xs font-semibold text-muted-foreground">
                        Conversations ({acceptedConversations.length})
                      </p>
                    </div>
                  )}
                </>
